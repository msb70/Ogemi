...
function ReportesPage({ scope }: { scope: ReporteScope }) {
  ...
  const loadAll = useCallback(async () => {
    ...
    setVentasOgemi(ventasOgemiData || [])
  ...
  // Ogemi: las ventas viven en ventas_ogemi; se mapean al shape de facturas que consumen las pestañas
  const hoyISO = new Date().toISOString().split('T')[0]
  const ventasOgemiMap = ventasOgemi.map((v: any) => ({
    ...v,
  ...
  const carteraOgemi: CarteraVencida[] = ventasOgemiMap
    ...
    .map((v: any) => {
      ...
      const tramo = dias <= 0 ? 'corriente' : dias <= 30 ? '1-30' : dias <= 60 ? '31-60' : dias <= 90 ? '61-90' : dias <= 120 ? '91-120' : '+120'
      return {
  ...
  const ventasBase = esOgemi ? ventasOgemiMap : facturas
  const carteraScope = esOgemi ? carteraOgemi : cartera
  const ventas = ventasBase.filter(f => !isNC(f.tipo_documento))
  ...
  return (
    <AppLayout>
      <Header title={tituloPagina} subtitle={subtituloPagina}
        actions={
        ...
        <div className="hidden print:block px-6 pt-6 mb-2">
          <div className="flex items-center gap-3 border-b-2 pb-3" style={{ borderColor: '#0f766e' }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 11, color: '#6b7280' }}>{marcaPrint}</div>
            </div>
