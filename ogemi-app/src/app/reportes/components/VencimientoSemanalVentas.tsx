...
import { getNextFridays, buildVencimientoViernes } from '../reportes.utils'
...
export interface VencimientoSemanalVentasProps {
  facturas: any[]
...
export default function VencimientoSemanalVentas({
  ...
  const weekDates = weekDatesProp ?? internalDates
        ...
        {weekDateObjs.map((_, i) => {
          ...
          return (
            <div key={i} className={`card p-4 border-t-4 ${c.bg} ${c.border}`}>
              ...
              {datesReadOnly ? (
                <p className="text-xs text-gray-600 mt-0.5 mb-2">{formatDate(weekDates[i])}</p>
              ) : (
