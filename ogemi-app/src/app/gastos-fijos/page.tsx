...
function GastosFijosPage() {
  ...
  const guardarMonto = async (gastoId: string, diaCorte: 15 | 30) => {
    const key = `${gastoId}-${diaCorte}`
    const rawValue = montos[gastoId]?.[diaCorte] || '0'
    ...
    setSavingCell(null)
    if (error) {
      showToast(`Error al guardar monto: ${error.message}`, 'error')
      return
    }
        ...
        <section className="card overflow-hidden">
          <div className="px-4 py-3 bg-gray-50 border-b border-gray-200 flex items-center justify-between">
            ...
            <div className="flex gap-4 text-xs font-semibold text-gray-600">
              ...
              <span>Total: {formatCurrency(total15 + total30)}</span>
            </div>
          </div>
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-100">
                {loading ? (
                  ...
                  <tr><td colSpan={5} className="text-center py-10 text-gray-400">No hay gastos fijos creados.</td></tr>
                ) : (
                  gastos.map(gasto => (
                          ...
                          <td key={diaCorte} className="table-cell">
                            <div className="flex justify-end gap-2">
                              <input
                                ...
                                onBlur={() => guardarMonto(gasto.id, diaCorte)}
                                disabled={!gasto.activo}
                              />
                              <button
                                className="btn-secondary p-2"
                                title={`Guardar corte ${diaCorte}`}
                                onClick={() => guardarMonto(gasto.id, diaCorte)}
                                disabled={savingCell === key || !gasto.activo}
                              >
                                <Save size={14} />
                              </button>
                            </div>
              ...
              {gastos.length > 0 && (
                <tfoot>
                  <tr className="border-t border-gray-200 bg-gray-50">
                    ...
                    <td className="table-cell font-bold">{formatCurrency(total15 + total30)}</td>
                  </tr>
                </tfoot>
              )}
            </table>
